...
public class ListenerAntiCheat implements Listener {
	...
	Material[] pressure_plates = {Material.ACACIA_PRESSURE_PLATE, Material.BIRCH_PRESSURE_PLATE, 
			...
			Material.OAK_PRESSURE_PLATE, Material.SPRUCE_PRESSURE_PLATE, Material.STONE_PRESSURE_PLATE,
	...
	public void guardianSpawn(EntitySpawnEvent e) {
		if (e.getEntityType() == EntityType.GUARDIAN && e.getLocation().getBlock().getRelative(BlockFace.DOWN).getType() != Material.WATER) {
			e.setCancelled(true);
		}
	}
	...
	public void monsterSpawnerSpawn(CreatureSpawnEvent e) {
		if (e.getSpawnReason() == SpawnReason.SPAWNER) {
			if (Math.random() > spawner_chance) e.setCancelled(true);
		}
	}
